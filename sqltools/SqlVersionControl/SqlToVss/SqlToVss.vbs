...
CONST VSSPath = "C:\dev\sql_to_vss\vss_db\srcsafe.ini"
CONST VSSRoot="$/sql_to_vss" ' note if you are going to use the root of your Source Safe Database use $, not $/
CONST VSSAdminAccount = "admin"
CONST VSSAdminAccountPassword = ""
CONST FileRepositoryPath = "C:\dev\sql_to_vss\sql_servers"
...
'normalize SQL Server Instance Names
...
if flag=0 then
	...
	if ErrorNumber=0 then
		...
		for each database in objSQLServer.Databases
			...
			'figure out if database is accessible...
...
Sub ConnectToSQL
	...
	' if not username then assume integrated authentication
	if wscript.arguments(1) = "" then
		objSQLServer.LoginSecure = True
		obJSQLServer.Connect ServerName
	else
		objSQLServer.LoginSecure = False
		obJSQLServer.Connect ServerName, wscript.arguments(1), wscript.arguments(2)
	end if
